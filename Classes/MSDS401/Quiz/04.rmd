...
pHat2 <- x2 / n2
pHat <- (x1 + x2) / (n1 + n2)
pretty_vector(round(pHat, 4))

zScore <- (pHat2 - pHat1) / (sqrt((pHat * (1 - pHat)) * ((1 / n1) + (1 / n2))))

pretty_vector(round(zScore, 3))
...
lm.intercept = coef(model)[[ 1 ]]
lm.slope <- coef(model)[[ 2 ]]

ggplot(data, aes(x, y)) +
    geom_point() +
	geom_abline(intercept = lm.intercept, slope = lm.slope, lwd = 2)
