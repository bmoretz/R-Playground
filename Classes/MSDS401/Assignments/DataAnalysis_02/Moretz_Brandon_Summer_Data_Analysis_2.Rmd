...
max.difference <- data.table(Volume = volume.value[which.max(smooth.difference)],
		TPR = (1 - prop.adults)[which.max(smooth.difference)], # [1] 0.7416332
		FPR = (1 - prop.infants)[which.max(smooth.difference)],
		Yield = sum(mydata$Volume > max.difference.volume) / nrow(mydata))
...
zero.a1.infants <- data.table(Volume = zero.a1.volume,
...
equal.error <- data.table(Volume = equal.error.volume,
...
ggplot() +
	...
	geom_text(aes(x = equal.error$FPR - .1, y = equal.error$TPR + .0), label = paste("equal harvest\nvol =", round(equal.error$Volume, 1))) +
