...
pretty_kable(results, "Standard Deviations or Skewness")
...
bootstrap.quantiles <- raw.bootstrap[, lapply(.SD, FUN = function(x) { quantile(x, c(.025, 0.975)) })]
bootstrap.variance <- data.table(Value = var(raw.bootstrap))

head(bootstrap.variance)

bootstrap.quantiles.melted <- melt
q3a_cap <- "MSDS 401: R Programming 2, Q3.b"

# Bootstrap Hist

getValueHist(bootstrap.variance, "Bootstrap Q", q3a_cap)
