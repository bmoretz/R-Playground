...
+ Sort the vector created in __(1)(a)__ in ascending order.  Output this result.  
...
+ Add this descending sequence arithmetically the sorted vector. This is __vector addition__, _not vector combination_. Output the contents. 
...
This problem requires finding the point of intersection of two functions. 
...
fun.values$Type <- ifelse(fun.values$variable == 'diff', 'solid', 'dashed')

fun.intercept <- fun.values[variable == "diff" & value == 0,][, .(x, y = value)]

ggplot(fun.values, aes(x = x, y = value, color = variable, linetype = Type, group = as.factor(variable))) +
    geom_line(lwd = 1.4) +
    geom_point(aes(x = fun.intercept$x, y = fun.intercept$y), size = 2.5, color = "#8E44AD", alpha = .8) +
    geom_vline(xintercept = fun.intercept$x, linetype = 'dashed', alpha = .5) +
    ...
    labs(title = "Function Intersection",
       subtitle = paste("Intersect: x =", fun.intercept$x, ", y =", fun.intercept$y),
...
+ Use *apply()* to return the median values for the three variables. Output these values.  
...
ggplot(trees, aes(Radius, fill = ..count..)) +
	...
	labs(title = "Tree Radius Distribution",
...
ggplot(trees, aes( "", y = Area)) +
	geom_boxplot( outlier.colour = "red", outlier.shape = 1, notch = T) +
    coord_flip() +
...
max_vol > ex_outlier_range

boxplot.stats(trees$Volume, coef = ex_outlier_coef)

head(trees[order(-trees$Area)], 1) # 1 liner
...
mtext("QQ Plot, Normal & Exponential (base R)", side = 3, line = -1.5, outer = TRUE)

par(opar)

# ggplot

# qqline equivilant function using ggplot & geom_abline. 
# This should match base R functionaliy from my understanding of the documentation.
...
q1 <- ggplot(values, aes(sample = x)) +
	...
	labs(title = "Normal Distribution", x = "", Y = "Normal Values")

q2 <- ggplot(values, aes(sample = y)) +
	stat_qq() +
	geom_qqline(values$y, color = "blue", alpha = .8 ) +
	labs(title = "Exponential Distribution", x = "", Y = "Exponential Values")

grid.arrange(q1, q2, ncol = 2, top = "QQ Plot, Normal & Exponential (ggplot)")

boxplot.stats(values$x, coef = ex_outlier_coef)
boxplot.stats(values$y, coef = ex_outlier_coef)
