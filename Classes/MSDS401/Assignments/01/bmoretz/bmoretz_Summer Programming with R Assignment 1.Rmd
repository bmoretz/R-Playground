...
fun.values <- melt(data.table(x, y1 = f1(x), y2 = f2(x), diff = abs(y1 - y2)), id = "x")

intercept <- min( fun.values[,fun.values$variable == "diff"] )

ggplot(fun.values, aes(x = x, y = value, color = variable, group = as.factor(variable), show.legend = T)) +
	geom_line(lwd = 1.4) +
    scale_fill_manual(values = c('y1', 'y2', 'diff'), labels = c('1', '2', '3')) +
    scale_color_manual(name = "variable", values = c("#9D1F04", "#479C67", "#3F2188"))
