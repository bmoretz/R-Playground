...
q1 <- ggplot(values, aes(sample = x)) +
	geom_qq() +
	geom_abline(intercept = mean(values$x), slope = sd(values$x), color = "blue", lwd = 1.5, alpha = 0.3) +
	labs(title = "Normal Distribution")

q2 <- ggplot(values, aes(sample = y)) +
	geom_qq() +
	geom_abline(intercept = mean(values$y), slope = sd(values$y), color = "blue", lwd = 1.5, alpha = 0.3) +
	labs(title = "Exponential  Distribution")

grid.arrange(q1, q2, ncol = 2, top = "QQ Plot X & Y")
