...
binom.est <- function(x, n, p, m, lower.tail = T ) {

  exp <- n * p
  std <- exp * (1 - p)

  ifelse(x < m, pnorm((x - exp + 0.5) / sqrt(std)), pnorm(x - exp - 0.5, exp, sqrt(std), lower.tail))
}

binom.est(0, n, p, m)
binom.est(6, n, p, m, lower.tail = F)
binom.est(10, n, p, m, lower.tail = F)

pnorm( ( 0 - exp + 0.5 ) / sqrt(std) )
pnorm((6 - exp + 0.5) / sqrt(std))
