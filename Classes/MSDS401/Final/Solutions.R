...
#     How many different groups of 4 are possible?  Selection order is unimportant.
...
#     How many different groups of 4 are possible?  Selection order is important.

factorial(30) / factorial(30 - 4) # [1] 657720

choose(30,4)*factorial(4)  # [1] 657720 
...
###########
# 2)  For the given probability distribution, find the mean, variance, 
...
# The mode is the most frequent value.  Here it is 0.
# The median is 2.  The probability of less than 2 is 0.49 and the probability of
# 2 or more is 0.51.  This satisfies the definition.  Note the following plot.
...
(8 + 17 + 7 + 37) / (8 + 17 + 18 + 13 + 7 + 37)  
# OR, 1 - Person_votes_Democrat [1] 0.69
...
###########
#5) Use the results summarized in the table for (4) and test at the 5% level if party 
#   affiliation and approval of the Mayor are independent of each other using 
...
X <- sum(X[,1]+X[,2])
X                                   # 14.63252
pchisq(X,2,lower.tail = FALSE)      # 0.0006646447

###########
# 6) A police department reports the probabilities that 0, 1, 2, and 3 burglaries
...
# The mode is the most frequent value.  Here it is 0.
# The median is 1.  The probability of less than 1 is 0.46 and the probability of
# 1 or more is 0.54.  This satisfies the definition.  Note the following plot.
...
#     6 pounds and 12 pounds, and is spread evenly forming a uniform distribution.
#     Find the probability of between 8.5 and 10 poundslost, and between 10 and 14 
#     pounds. Round to two decimal places.

(10 - 8.5) / (12 - 6) # [1] 0.25

round((12 - 10) / (12 - 6), digits = 2)  # [1] 0.33

###########
# 8) Find the indicated z-score. The graph depicts the standard normal distribution 
...
###########
# 9)  Here are two discrete probability distributions:   dist_A and dist_B.  By 
#     construction each may be either a binomial or a Poisson distribution. The 
#     mean and variance for dist_A are 5.00 and 5.00.  The mean and variance for
#     dist_B are 5.00 and 3.33.   Pick the best answer.     
#     (A)  Dist_A is Poisson, Dist_B is binomial
#     (B)  Dist_A is binomial, Dist_B is Poisson.
#     (C)  Both are Poisson
#     (D)  Both are binomial

# Generate the distributions shown in the problem.
x <- seq(0,15)
...
###########
#10   This problem involves estimating binomial probabilities for a binomial 
#     distribution with p = 0.4 and n = 15.  If X is a binomial random variable 
#     with these parameters, estimate the probabilities for the following three  
#     outcomes; X = 0, X = 6 and X >= 10.  Use the normal approximation with
#     continuity correction to do so.  

# Actual binomial probabilities
x <- seq(0,15)
n <- 15
p <- .4
...
sum(dbinom(seq(10,n),n,p))

# Solution
exp <- n*p
std <- exp*(1-p)
pnorm((0-exp+0.5)/sqrt(std),0,1,lower.tail=TRUE)
...
###########
# 12)	True or False:  in a hypothesis test regarding a population mean, the
...
#     sample size is increased and the probability of a type II error is fixed 
#     and does not change then the type I error rate does not change.

# FALSE. Keeping the power the same, an increase in sample size provides more data 
# for the statistical test making it less likely that a type I error will occur.  

###########
# 14)	True or False:  In a hypothesis test regarding a population mean, if the
#     sample size is increased and the probability of a type I error is fixed and 
#     does not change then the type II error rate will decrease.  

# TRUE. With alpha constant, an increase in sample size provides more data for the 
# statistical test increasing the power.  Thus it less likely that a type II error 
# will occur.

###########
# 15)	Suppose that you perform a hypothesis test regarding a population mean, and that
#     the evidence does not warrant rejection of the null hypothesis. When formulating
#     the conclusion to the test, why is phrase "fail to reject the null hypothesis" 
#     more accurate than the phrase "accept the null hypothesis?"  Give a short answer.
...
###########
# 16)	According to a recent poll, 53% of Americans would vote for the incumbent president.
...
###########
# 17)	What does the linear correlation coefficient tell us regarding the usefulness
#     of a regression equation for making predictions?
...
###########
# 18)  Assume that a hypothesis test of the given claim will be conducted. A cereal
...
###########
# 19)	Scores on a test are normally distributed with a mean of 68.2 and a standard
...
# Note that this is a two-step problem involving the normal distribution and the 
# binomial distribution which is used to calculate the second probability.
...
###########
# 20) Find the value of the linear correlation coefficient, r.

x <- c(62, 53, 64, 52, 52, 54, 58)
y <- c(158, 176, 151, 164, 164, 174, 162)

cor(x, y) # [1] -0.7749395
cor.test(x,y)  # p-value = 0.04071, reject

###########
# 21) A researcher wants to estimate what proportion of U.S. refinery workers are contract 
#     workers. The researcher wants to be 95% confident of her results and be within 0.05 
#     of the actual proportion. There are no prior studies.  The researcher has no idea 
#     what is the actual propulation proportion.  How large a sample size should be taken?  
...
# 22)	Use the given sample data to test the claim that P1 > P2. Use alpha = 0.01.
...
# evidence to support the claim that P1 > P2.
...
# 23)	 A researcher was interested in comparing the amount of time (in hours) spent
...
# 24)	A researcher was interested in comparing the amount of time spent watching
...
###########
# 25)  Construct the indicated confidence interval for the difference between population
...
c2 <- c2 - c1
x <- cbind(c1, c2)
xqt(0.054861/2,1,lower.tail = TRUE)

prop.test(x, alternative = c("two.sided"), conf.level = 0.90, correct = FALSE)

############
# 26)  Use the given data to find the equation of the regression line. Round the
...
# 27)  Using the data in problem 24, construct 95% confidence intervals for the 
# coefficients in the resulting regression model.

object <- lm(y~x)
summary(object)
# Coefficients:
#             Estimate  Std. Error  t value  Pr(>|t|)    
# (Intercept) -3.79005    1.23734  -3.063    0.054861 .  
# x            0.89745    0.05447  16.476    0.000487 ***

confint(object)
# 2.5 %    97.5 %
#  (Intercept) -7.7278276 0.1477305
#  x            0.7241004 1.0708025

-3.7900485 + qt(0.975,3,lower.tail=T)*1.23734  #Upper bound
-3.7900485 + qt(0.025,3,lower.tail=T)*1.23734  #Lower bound
0.89745 + qt(0.975,3,lower.tail=T)*0.05447     #Upper bound
0.89745 + qt(0.025,3,lower.tail=T)*0.05447     #Lower bound

###########
# 28) Conduct a chi-square test for independence. State the null and alternative
...
###########
# 29)	An urgent care center is interested in whether visits per day of the week are
#     uniformly distributed.Conduct a chi-square goodness of fit test. State the null
#     and alternative hypotheses, and provide the chi-square and associated p-value.
...
###########
# 30) Test the hypothesis of a zero correlation using the procedure shown
...
###########
# 31)	A project manager is trying to assemble a five-member team. She has a
...
###########
# 32)	Consumers are asked to rate a company both before and after viewing a video 
...
###########
# 33)  Random samples of men from two different towns were obtained and the weights 
# of the men measured in pounds.  Using the data below test the claim there is more 
# variation in weights of men from town A versus the weights of men from town B.  
...
dfB <- nB-1
F <- (sA/sB)^2    # 1.181867
qf(0.95, dfA, dfB,lower.tail = TRUE)   # 1.993819
pf(F, dfA, dfB, lower.tail = FALSE)    # 0.3518687

#  Do not reject at the 5% significance level.

###########
# 34)  Use the p-value method to test the claim that the population standard 
#      deviation of the systolic blood pressures of adults aged 40-50 is equal 
#      to 22 mmHG.  Use the following sample statistics:  n = 23, sample mean = 
#      132.3 mmHg, and sample standard deviation s = 26.6 mmHg.  Determine the 
#      value of the test statistic, the p-value and your conclusion.  Use a 5% 
#      significance level.  Assume normality. This is a two-sided test. Also, 
#      calculate a 95% confidence interval for the population standard deviation.

n <- 23
chi <- (n-1)*(26.6/22)^2
chi
pchisq(chi, n-1, lower.tail = FALSE)  # 0.07467875

# Since the p-value is greater than 2.5%, the null hypothesis of variance
# equality can not be rejected.

# First determine the critical values for a chi-square statistic with n-1 degrees
# of freedom.  Then invert the chi-square statistic solving for the variance.

critical.values <- c(qchisq(0.975,n-1,lower.tail=T), qchisq(0.025,n-1,lower.tail=T))
critical.values
sqrt(((n-1)*26.6^2)/critical.values)

###########
# 35)	Fill in the missing entries in the following, partially completed one-way
#     ANOVA table.  Determine the p-value and the critical value for the F statistic
#     at the 1% level.
...
qf(0.99, 3, 20, lower.tail = TRUE)    # critical value = 4.938
